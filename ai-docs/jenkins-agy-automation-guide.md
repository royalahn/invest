...
## 수집 단계 (Deep Research)
다음 주제에 대해 웹에서 '종가(마감)' 기준 최신 뉴스와 데이터를 '매우 상세하게' 검색해줘:
1. 한국 시장: KOSPI, KOSDAQ 지수 마감 동향 (반드시 네이버 금융 또는 한국거래소 공식 기사 참조), 주요 3개 업종별 흐름, 외국인/기관 수급의 구체적 규모
2. 미국 시장: S&P500, NASDAQ 마감 동향 (반드시 Yahoo Finance 또는 CNBC 공식 수치 참조), M7(빅테크) 종목별 구체적 마감 등락률
3. 매크로: 한미 금리 동향, 환율(USD/KRW), 유가 (마감 기준)

⚠️ 주의: 반드시 장 마감 후의 '종가' 데이터를 기반으로 수치를 작성하고, 수치가 불확실한 경우 대략적인 값이나 과거 데이터를 날조(Hallucination)하지 마세요.
